...
VIEWPORT_W = 600#200 for demo
VIEWPORT_H = 400 #1000 for demo
...
class BipedalWalkerMovement(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(self):
        ...
        self.fd_obstacle_polygon = fixtureDef(
                        shape = polygonShape(vertices=
                        [(0, 0),
                         (1, 0),
                         (1, -1),
                         (0, -1)]),
                        friction = FRICTION)
        self.obstacle_polygon = [(-0.5,0),(-0.5,0.25),(-0.25,0.5),(0.25,0.5),(0.5,0.25),(0.5,0)]#[(-30,+9), (+6,+9), (+34,+1), (+34,-8), (-30,-8)]
    ...
    def my_init(self, params):
        ...
        if leg_size == "short":
            LEG_W = 8 / SCALE
            LEG_H = 17 / SCALE
        elif leg_size == "long": # long legged has 2 pairs of legs and bigger body

            MOTORS_TORQUE = 300 #400 # increase motor torque for big boy
        ...
        high = np.array([np.inf] * (4 + self.nb_leg_pairs*2*5 + NB_LIDAR))
        self.observation_space = spaces.Box(-high, high, dtype=np.float32)
    ...
    def set_environment(self, roughness=None, stump_height=None, stump_width=None, stump_rot=None,
                        obstacle_spacing=None, gap_width=None, step_height=None, step_number=None, tunnel_height=None,
                        env_param_input=False, poly_shape=None, stump_seq=None, moving_stumps=None):
        #print('SETTING ENV')
        ...
        if moving_stumps is not None:
            self.moving_stumps = moving_stumps
            self.obstacle_spacing = 1
        if poly_shape is not None:
            self.poly_shape = np.interp(poly_shape,[0,4],[0,4]).tolist()
            assert((len(poly_shape) == 12)
                   or (len(poly_shape) == 15)
                   or (len(poly_shape) == 20)
                   or (len(poly_shape) == 30))
            self.poly_shape = self.poly_shape[0:12]
    ...
    def _generate_terrain(self, hardcore):
        ...
        GRASS, STUMP, STAIRS, PIT, TUNNEL, POLY, SEQ, MOVE = 0, None, None, None, None, None, None, None
        cpt = 1
        if self.step_height:
            STAIRS = cpt
            cpt += 1
        ...
        if self.stump_seq is not None:
            SEQ = cpt
            cpt += 1
        if self.moving_stumps is not None:
            speed = self.moving_stumps[0]
            MOVE = cpt
            cpt += 1
        _STATES_ = cpt
        ...
        # add startpad
        max_startpad_x = TERRAIN_STARTPAD*TERRAIN_STEP
        ...
        while x < max_x:
            ...
            if state==GRASS and not oneshot:
                ...
                if x > max_startpad_x: velocity += self.np_random.uniform(-self.roughness, self.roughness)/SCALE   #1
                y += velocity
            ...
            elif state==POLY and oneshot:
                # first point do not move
                poly = []
                delta_pos = []
                for i in range(0,len(self.poly_shape),2):
                    delta_pos.append(tuple(np.random.normal(self.poly_shape[i:i+2],0.1)))
                for i,(b,d) in enumerate(zip(self.obstacle_polygon, delta_pos)):
                    if i != 0 and i != (len(self.obstacle_polygon)-1):
                        ...
                        poly.append((x + (b[0] * TERRAIN_STEP) + (d[0]*TERRAIN_STEP),
                                     y + (b[1] * TERRAIN_STEP)))
                x += 1
                self.fd_obstacle_polygon.shape.vertices = poly
                t = self.world.CreateStaticBody(
                    fixtures=self.fd_obstacle_polygon)
                ...
                x += stump_width
                self.fd_polygon.shape.vertices = poly
                t = self.world.CreateKinematicBody(
                    fixtures=self.fd_polygon,
                    userData=True)
                speed += 0.1
                t.linearVelocity = (0, np.cos(speed)*5)
                t.color1, t.color2 = (1, 1, 1), (0.6, 0.6, 0.6)
                self.terrain.append(t)
    ...
    def draw_walker(self):
        ...
        for x_anchor in self.leg_pairs_x:
            absolute_x = init_x + np.interp(x_anchor,[-1,1],[-HULL_WIDTH/2/SCALE,HULL_WIDTH/2/SCALE])
            for i in [-1, +1]:
                leg = self.world.CreateDynamicBody(
                    position=(absolute_x, init_y - LEG_H / 2 - LEG_DOWN),
                    angle=(i * 0.05),
                    fixtures=LEG_FD
                )
                ...
                rjd = revoluteJointDef(
                    bodyA=self.hull,
                    bodyB=leg,
                    localAnchorA=(x_anchor, LEG_DOWN),
                ...
                lower = self.world.CreateDynamicBody(
                    position=(absolute_x, init_y - LEG_H * 3 / 2 - LEG_DOWN),
                    angle=(i * 0.05),
                    fixtures=LOWER_FD
                )
    ...
    def reset(self):
        #   print(self.total_torque_penalty)
        self.total_torque_penalty = 0
        self._destroy()
    ...
    def update_stumps(self):
        pass
        # for k,t in enumerate(self.terrain):
        #     pass
        #     if t.userData:
        #         old_speed = t.linearVelocity[1]/5
        #         assert old_speed <= 1.0
        #         assert old_speed >= -1.0
        #         if k==116: print(np.cos(np.arccos(old_speed) + 0.01 * np.sign(old_speed)))
        #         t.linearVelocity[1] = np.cos(np.arccos(old_speed) + 0.01 * np.sign(old_speed))*5
        #         if t.position[1] > 2 or t.position[1] < -2:
        #             t.linearVelocity[1] *= -1
    ...
    def step(self, action):
        ...
        if control_speed:
            ...
            for i in range(len(self.joints)):
                if i%2 == 0:
                    ...
                    self.joints[i].motorSpeed = float(SPEED_KNEE * np.sign(action[i]))
        ...
        for i in range(0,len(self.legs),2):
            state += [self.joints[i].angle,   # gives 1.1 on high up (spikes on hiting the ground, that's normal too)
                      self.joints[i].speed / SPEED_HIP,
                      self.joints[i+1].angle + 1.0,
                      self.joints[i+1].speed / SPEED_KNEE,
        ...
        for a in action:
            ...
            self.total_torque_penalty += self.torque_penalty * MOTORS_TORQUE * np.clip(np.abs(a), 0, 1)
            #self.tmpenalty.append(self.torque_penalty * MOTORS_TORQUE * np.clip(np.abs(a), 0, 1))
