...
class SAGG_RIAC():
    ...
    def __init__(self, min, max, temperature=20):
        ...
        self.interest = [0.]
        self.probas = [1.]
        self.tree.create_node('root','root',data=Region(maxlen=self.maxlen, cps_gs=[[],[]],
    ...
    def compute_interest(self, sub_region):
        if len(sub_region[0]) > 10:  # TRICK NB 4
            ...
            #                                                 diff,
            #                                                 cp))
            # cp = np.abs(np.array(sub_region[0])[-cp_window:].mean())
            # print(cp)
        else:
            cp = 0
        interest = np.abs(cp)
    ...
    def split(self, nid):
        ...
        for i in range(self.nb_split_attempts):
            ...
            if split_score >= best_split_score \
              and np.abs(interest[0] - interest[1]) >= self.max_difference / 8 \
              and valid_bounds: # TRICK NB 3, max diff
                is_split = True
    ...
    def update(self, goal, continuous_competence, all_raw_rewards):
        ...
        # collect new interests
        self.interest = [n.data.interest for n in self.tree.all_nodes()]
        exp_int = np.exp(self.temperature * np.array(self.interest)) # TRICK NB 5 TODO switch to prop sampling in future
        ...
        assert len(self.probas) == len(self.regions_bounds)
